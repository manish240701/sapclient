...
const NavComponent10 = () => {
  ...
  const [writtenTestCount, setWrittenTestCount] = useState(0);
  const [fileNames, setFileNames] = useState([]);

  const weightageCalculator = () => {
    if (category === "written test cleared") {
      return 5;
    }
    if (category === "placed") {
      return 40;
    }
    if (category === "placed with internship") {
      return 50;
    }
    if (category === "internship without placement") {
      return 20;
    }
  };

  useEffect(() => {
    if (category !== "empty" && category && file) {
      setAllFilled(true);
    }
  }, [category, file]);
  ...
  const handleUpload = async (e) => {
    e.preventDefault();
    if (writtenTestCount >= 4 && category === "written test cleared") {
      alert("Maximum 4 written tests options allowed");
      dataEmptifier();
      return;
    }

    if (categoryPoints + weightageCalculator() > 50) {
      ...
      (snapShot) => {
        ...
        snapShot.docs.forEach((doc) => {
          ...
          if (doc.data().activityCategory === categoryHeader) {
            ...
            count += 1;
            if (doc.data().category === "written test cleared") {
              testCount += 1;
            }
          }
        ...
        <form className="add-newdata-form" onSubmit={handleUpload}>
          <section className="form-group">
            ...
            >
              ...
              <option value="written test cleared">Written test cleared</option>
              <option value="placed">Placed</option>
              <option value="placed with internship">
                Placed with Internship
              </option>
              <option value="internship without placement">
                Internship without Placement
              </option>
            </select>
          </section>
          <section className="form-group">
