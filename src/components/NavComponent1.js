...
const NavComponent1 = () => {
      ...
      if (uploadStatus === "success") {
        try {
          ...
          await setDoc(
            doc(
              db,
              `alldept/${studentDept}/years/${studentYear}/sections/${studentSection}/allStudents/${studentId}/all`,
              id
            ),
            {
              ...uploadData,
    ...
    const uploadFile = () => {
      ...
      if (studentId !== null) {
        const storageRef = ref(
          storage,
          `${studentDept}/${studentYear}/${studentSection}/${studentId}/${file.name}`
        );
  ...
  const handleDelete = async (id, fileName) => {
    try {
      ...
      const desertRef = ref(
        storage,
        `${studentDept}/${studentYear}/${studentSection}/${studentId}/${fileName}`
      );
